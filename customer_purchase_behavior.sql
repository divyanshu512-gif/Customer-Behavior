...
SELECT subscription_status, COUNT(customer_id) AS counting, ROUND(AVG(purchase_amount),2) AS avg_amount, SUM(purchase_amount) AS revenue
FROM customer 
...
SELECT item_purchased,
       (100 * SUM(CASE WHEN discount_applied = 'Yes' THEN 1 ELSE 0 END)/COUNT(discount_applied),2) AS discount_percentage
FROM customer
GROUP BY item_purchased
...
SELECT customer_segment, count(*) AS customer_type_segment
FROM customer_type
...
SELECT subscription_status , COUNT(customer_id) AS numbers
FROM customer 
